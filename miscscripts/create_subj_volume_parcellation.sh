...
# Shell script. Copy this script to to your $SUBJECTS_DIR/ folder.
...
# Subject list. Create a list with the identifiers of the desired target subjects (named exactly as their corresponding names in $SUBJECTS_DIR/).
#  
# Instructions:
# 
...
while getopts ":L:f:l:a:d::m:t:" o; do
    case "${o}" in
        L)
            L=${OPTARG}
            ;;
...
if [ ! -e fsaverage/label/lh.${annotation_file}.annot ] 
	then cp lh.${annotation_file}.annot fsaverage/label/
fi
if [ ! -e fsaverage/label/rh.${annotation_file}.annot ] 
	then cp rh.${annotation_file}.annot fsaverage/label/
fi

# Convert annotation to label, and get color lookup tables
rm -f ./${output_dir}/log_annotation2label
mri_annotation2label --subject fsaverage --hemi lh --outdir ./${output_dir}/label --annotation ${annotation_file} >> ./${output_dir}/temp_${first}_${last}/log_annotation2label
mri_annotation2label --subject fsaverage --hemi lh --outdir ./${output_dir}/label --annotation ${annotation_file} --ctab ${output_dir}/temp_${first}_${last}/colortab_${annotation_file}_L1 >> ./${output_dir}/temp_${first}_${last}/log_annotation2label
mri_annotation2label --subject fsaverage --hemi rh --outdir ./${output_dir}/label --annotation ${annotation_file} >> ./${output_dir}/temp_${first}_${last}/log_annotation2label
mri_annotation2label --subject fsaverage --hemi rh --outdir ./${output_dir}/label --annotation ${annotation_file} --ctab ${output_dir}/temp_${first}_${last}/colortab_${annotation_file}_R1 >> ./${output_dir}/temp_${first}_${last}/log_annotation2label
...
echo $subject
	...
	if [[ -e ${subject}/label/lh.${subject}_${annotation_file}.annot ]] && [[ -e ${subject}/label/rh.${subject}_${annotation_file}.annot ]]
		...
		for labelsR in `cat ${output_dir}/temp_${first}_${last}/list_labels_${annotation_file}R`
			do printf " --l ${output_dir}/${subject}/label/${labelsR}" >> ${output_dir}/temp_${first}_${last}/temp_cat_${annotation_file}_R
		done
		for labelsL in `cat ${output_dir}/temp_${first}_${last}/list_labels_${annotation_file}L`
			do if [ -e ${output_dir}/${subject}/label/${labelsL} ]
				then printf " --l ${output_dir}/${subject}/label/${labelsL}" >> ${output_dir}/temp_${first}_${last}/temp_cat_${annotation_file}_L
			fi
		done
	
		mris_label2annot --s ${subject} --h rh `cat ${output_dir}/temp_${first}_${last}/temp_cat_${annotation_file}_R` --a ${subject}_${annotation_file} --ctab ${output_dir}/temp_${first}_${last}/colortab_${annotation_file}_R >> ${output_dir}/${subject}/label2annot_${annotation_file}rh.log 
		mris_label2annot --s ${subject} --h lh `cat ${output_dir}/temp_${first}_${last}/temp_cat_${annotation_file}_L` --a ${subject}_${annotation_file} --ctab ${output_dir}/temp_${first}_${last}/colortab_${annotation_file}_L >> ${output_dir}/${subject}/label2annot_${annotation_file}lh.log 

	fi

	# Convert annot to volume
	rm -f ${output_dir}/${subject}/log_aparc2aseg
	mri_aparc2aseg --s ${subject} --o ${output_dir}/${subject}/${annotation_file}.nii.gz  --annot ${subject}_${annotation_file} >> ${output_dir}/${subject}/log_aparc2aseg

	# Create individual mask files
	if [[ ${create_individual_masks} == "YES" ]]
		then 
		echo "Creating individual region masks for subject ${subject}"
		mkdir -p ${output_dir}/${subject}/masks
		for ((i=1;i<=${number_labels_L};i+=1))
			do num=`echo "${i}+1000" | bc`
			echo "Mask left hemisphere: $i"
			sed -n "$i,$i p" ${output_dir}/temp_${first}_${last}/list_labels_${annotation_file}L1 > ${output_dir}/temp_${first}_${last}/temp_regionL
			temp_region=`cat ${output_dir}/temp_${first}_${last}/temp_regionL`
		...
		for ((i=1;i<=${number_labels_R};i+=1))
			...
			echo "Mask right hemisphere: $i"
			sed -n "$i,$i p" ${output_dir}/temp_${first}_${last}/list_labels_${annotation_file}R1 > ${output_dir}/temp_${first}_${last}/temp_regionR
			temp_region=`cat ${output_dir}/temp_${first}_${last}/temp_regionR`
			fslmaths ${output_dir}/${subject}/${annotation_file}.nii.gz -thr ${num} -uthr ${num} -bin ${output_dir}/${subject}/masks/${temp_region}
		done
	...
	if [[ ${get_anatomical_stats} == "YES" ]]
		...
		mris_anatomical_stats -a $SUBJECTS_DIR/${subject}/label/lh.${subject}_${annotation_file}.annot -b ${subject} lh > ${output_dir}/${subject}/tables/table_lh.txt
		mris_anatomical_stats -a $SUBJECTS_DIR/${subject}/label/rh.${subject}_${annotation_file}.annot -b ${subject} rh > ${output_dir}/${subject}/tables/table_rh.txt
		
		# Get tables with numerical values only
		sed '1,15d' ${output_dir}/${subject}/tables/table_rh.txt > ${output_dir}/${subject}/tables/table_rh_values
		sed '1,15d' ${output_dir}/${subject}/tables/table_lh.txt > ${output_dir}/${subject}/tables/table_lh_values
		sed -i -r 's/\S+//10' ${output_dir}/${subject}/tables/table_lh_values
		sed -i -r 's/\S+//10' ${output_dir}/${subject}/tables/table_rh_values

		# Get variable names
		sed '1,5d;15,$d' ${output_dir}/${subject}/tables/table_rh.txt > ${output_dir}/temp_${first}_${last}/rh_mri_anatomical_stats_variables.txt
		cut -c5- ${output_dir}/temp_${first}_${last}/rh_mri_anatomical_stats_variables.txt > ${output_dir}/temp_${first}_${last}/rh_mri_anatomical_stats_variables2.txt 
		sed -i 's/ /_/g' ${output_dir}/temp_${first}_${last}/rh_mri_anatomical_stats_variables2.txt        
		awk '{ for (f = 1; f <= NF; f++)   a[NR, f] = $f  }  NF > nf { nf = NF } END {   for (f = 1; f <= nf; f++) for (r = 1; r <= NR; r++)     printf a[r, f] (r==NR ? RS : FS)  }' ${output_dir}/temp_${first}_${last}/rh_mri_anatomical_stats_variables2.txt > ${output_dir}/${subject}/tables/rh_mri_anatomical_stats_variables.txt

		sed '1,5d;15,$d' ${output_dir}/${subject}/tables/table_lh.txt > ${output_dir}/temp_${first}_${last}/lh_mri_anatomical_stats_variables.txt
		cut -c5- ${output_dir}/temp_${first}_${last}/lh_mri_anatomical_stats_variables.txt > ${output_dir}/temp_${first}_${last}/lh_mri_anatomical_stats_variables2.txt 
		sed -i 's/ /_/g' ${output_dir}/temp_${first}_${last}/lh_mri_anatomical_stats_variables2.txt        
		awk '{ for (f = 1; f <= NF; f++)   a[NR, f] = $f  }  NF > nf { nf = NF } END {   for (f = 1; f <= nf; f++) for (r = 1; r <= NR; r++)     printf a[r, f] (r==NR ? RS : FS)  }' ${output_dir}/temp_${first}_${last}/lh_mri_anatomical_stats_variables2.txt > ${output_dir}/${subject}/tables/lh_mri_anatomical_stats_variables.txt
	...
	echo "         >>>>         ${subject} STARTED AT `cat ${output_dir}/temp_${first}_${last}/start_date`, ENDED AT: $(date)        <<<<"
